...
class UdajovkyTests: XCTestCase {
    ...
    func testReferencesInStaticStructure() throws {
        ...
        XCTAssert(tree.root === list.first)
        XCTAssert(tree.root?.leftSon === list[1])
        XCTAssert(tree.root?.rightSon === list[4])
        XCTAssert(tree.root?.leftSon?.leftSon === list[2])
        XCTAssert(tree.root?.leftSon?.rightSon === list[8])
        XCTAssert(tree.root?.leftSon?.leftSon?.rightSon?.rightSon === list[9])
        XCTAssert(tree.root?.leftSon?.rightSon?.rightSon?.rightSon === list[14])
        XCTAssert(tree.root?.rightSon?.leftSon?.leftSon === list.last)
    }
